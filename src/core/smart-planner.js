...
(() => {
  ...
  function inferRequiredFilenames(item) {
    ...
    const add = (value) => {
      const filename = String(value || '')
        .replace(/^[`'"(<\[]+|[`'".,;:)\]]+$/g, '')
        .trim();
      if (!/^[a-z0-9][a-z0-9._-]{1,80}\.(?:py|java|json|js|ts|tsx|jsx|cpp|cc|c|h|hpp|swift|kt|rb|go|rs|sql|ipynb|rmd|md|pdf|docx?|pptx?|xlsx?|csv|zip|tar|gz)$/i.test(filename)) return;
    ...
    for (const match of raw.matchAll(/(?:submit|upload|attach|include|turn in|turn-in|deliverable(?:s)?|file(?:s)?(?: named)?|called|named)\s+(?:the\s+)?[`'"(\[]?([a-z0-9][a-z0-9._-]{1,80}\.(?:py|java|json|js|ts|tsx|jsx|cpp|cc|c|h|hpp|swift|kt|rb|go|rs|sql|ipynb|rmd|md|pdf|docx?|pptx?|xlsx?|csv|zip|tar|gz))/gi)) {
      add(match[1]);
    }
    for (const match of raw.matchAll(/[`'"]([a-z0-9][a-z0-9._-]{1,80}\.(?:py|java|json|js|ts|tsx|jsx|cpp|cc|c|h|hpp|swift|kt|rb|go|rs|sql|ipynb|rmd|md|pdf|docx?|pptx?|xlsx?|csv|zip|tar|gz))[`'"]/gi)) {
      add(match[1]);
    }
  ...
  function inferFreshnessWarnings(item, nowMs = Date.now()) {
    const warnings = [];
    const add = (label) => { if (label && !warnings.includes(label)) warnings.push(label); };
    const dueTs = Number(item?.ts || (item?.dueAt && new Date(item.dueAt).getTime()));
    const sourceUpdatedAt = item?.updatedAt || item?.updated_at || item?.modifiedAt || item?.modified_at || item?.lastUpdatedAt;
    const sourceSyncedAt = item?.syncedAt || item?.synced_at || item?.fetchedAt || item?.fetched_at || item?.indexedAt || item?.indexed_at;
    const sourceTime = Date.parse(sourceUpdatedAt || sourceSyncedAt || '');
    const previousDueAt = item?.previousDueAt || item?.previous_due_at || item?.cachedDueAt || item?.cached_due_at;
    const previousDueTs = Date.parse(previousDueAt || '');

    if (Number.isFinite(sourceTime)) {
      const ageHours = (nowMs - sourceTime) / (60 * 60 * 1000);
      if (ageHours >= 24 * 7) add('source may be stale: last Canvas update over 7 days ago');
      else if (Number.isFinite(dueTs) && dueTs - nowMs <= 48 * 60 * 60 * 1000 && ageHours >= 24) add('re-sync before planning: source older than 24h');
    } else if (Number.isFinite(dueTs) && dueTs - nowMs <= 48 * 60 * 60 * 1000) {
      add('freshness unknown: open Canvas before finalizing work');
    }

    if (Number.isFinite(previousDueTs) && Number.isFinite(dueTs) && Math.abs(previousDueTs - dueTs) >= 60 * 1000) {
      add('due date changed since last snapshot');
    }

    return warnings.slice(0, 3);
  }

  function buildStudyPackReviewSchedule(item, dueAt, generatedAt) {
    const createdMs = Date.parse(generatedAt);
    const dueMs = dueAt ? Date.parse(dueAt) : NaN;
    const source = `${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const steps = [];
    const add = (label) => { if (!steps.includes(label)) steps.push(label); };

    const daysUntilDue = Number.isFinite(createdMs) && Number.isFinite(dueMs)
      ? (dueMs - createdMs) / MS_DAY
      : Infinity;
    const isExamLike = /\b(exam|midterm|final|test|quiz|practice exam|study guide|flashcards?|anki|spaced repetition)\b/.test(source);
    const isProjectLike = /\b(project|lab|milestone|implementation|coding|programming|repo|github|autograder|gradescope|write[- ]?up|report)\b/.test(source);
    const isReadingLike = /\b(reading|chapter|lecture|slides?|notes?|paper|article|transcript|video)\b/.test(source);

    if (daysUntilDue <= 0) {
      add('now: final source check');
      add('after submit: log misses');
      return steps;
    }

    add('today: answer from memory, then verify source');
    if (daysUntilDue <= 2) {
      add('tomorrow: retry missed questions');
    } else if (daysUntilDue <= 7) {
      add('2 days: spaced recall pass');
      add('1 day before due: final check');
    } else {
      add('2 days: spaced recall pass');
      add('1 week: interleaved review');
      add('1 day before due: final check');
    }

    if (isExamLike) add('exam mode: practice under time');
    else if (isProjectLike) add('project mode: retest edge case');
    else if (isReadingLike) add('reading mode: teach-back summary');
  ...
  function buildAssignmentBriefMarkdown(item, dueAt = null) {
    ...
    const freshnessWarnings = inferFreshnessWarnings(item, Date.now());
    const lines = [
      `Assignment: ${title}`,
      ...
      workflowLabels.length ? `Workflow: ${workflowLabels.join(', ')}` : '',
      submissionModes.length ? `Submission mode: ${submissionModes.join(', ')}` : '',
      `Grounding: ${grounding}`,
      freshnessWarnings.length ? `Freshness: ${freshnessWarnings.join('; ')}` : '',
  ...
  function normalizeBriefDate(now = new Date()) {
    return now && typeof now.getTime === 'function' && Number.isFinite(now.getTime()) ? now : new Date();
  }

  function buildTodayCourseBrief(items, now = new Date()) {
    const generatedDate = normalizeBriefDate(now);
    const generatedAt = generatedDate.toISOString();
    const nowMs = generatedDate.getTime();
    const sourceItems = (Array.isArray(items) ? items : [items]).filter(Boolean)
      .map((item, index) => ({
        ...item,
        _index: index,
        ts: Number(item.ts || (item.dueAt && new Date(item.dueAt).getTime()))
      }))
      .filter(item => Number.isFinite(item.ts) && !item.done)
      .sort((a, b) => a.ts - b.ts || a._index - b._index);

    const dueSoon = sourceItems.filter(item => item.ts >= nowMs - MS_DAY && item.ts <= nowMs + 7 * MS_DAY);
    const overdue = sourceItems.filter(item => item.ts < nowMs);
    const priorityFilters = summarizePriorityFilters(sourceItems, nowMs);
    const deadlineClusters = summarizeDeadlineClusters(dueSoon, nowMs);
    const highLeverage = dueSoon
      .filter(item => classifyDeadline(item, nowMs).effort === 'high' || inferPlannerRiskFlags(item, dueSoon, nowMs).length)
      .slice(0, 5)
      .map(item => {
        const risks = inferPlannerRiskFlags(item, dueSoon, nowMs);
        const firstAction = recommendNextStudyAction(item, nowMs);
        return {
          title: String(item.title || 'Canvas item').trim(),
          course: String(item.courseName || item.course || '').trim(),
          dueAt: new Date(item.ts).toISOString(),
          estimatedMinutes: inferEstimatedWorkMinutes(item, nowMs),
          nextActionLabel: inferNextActionLabel(item, nowMs),
          workflowLabels: inferCsWorkflowTypeLabels(item),
          freshnessWarnings: inferFreshnessWarnings(item, nowMs),
          risks,
          firstAction
        };
      });
    ...
    const markdown = [
      '# Canvascope Today Brief',
      '',
      `Generated: ${generatedAt}`,
      '',
      `Due this week: ${dueSoon.length}`,
      overdue.length ? `Overdue: ${overdue.length}` : '',
      `Priority filters: ${formatPriorityFilterSummary(priorityFilters)}`,
      deadlineClusters.length ? `Deadline clusters: ${deadlineClusters.map(cluster => cluster.summary).join(' · ')}` : '',
      '',
      highLeverage.length ? '## High-leverage next actions' : '## High-leverage next actions',
      ...(highLeverage.length ? highLeverage.flatMap(item => [
        `- ${item.course ? `${item.course}: ` : ''}${item.title}`,
        `  - Due: ${item.dueAt}`,
        `  - Estimate: ${item.estimatedMinutes}m`,
        item.workflowLabels.length ? `  - Workflow: ${item.workflowLabels.join(', ')}` : '',
        item.freshnessWarnings.length ? `  - Freshness: ${item.freshnessWarnings.join('; ')}` : '',
        item.nextActionLabel ? `  - Next action: ${item.nextActionLabel}` : '',
        item.firstAction ? `  - First action: ${item.firstAction}` : '',
        item.risks.length ? `  - Watch: ${item.risks.join(', ')}` : ''
      ].filter(Boolean)) : ['- No urgent high-risk Canvas items detected.'])
    ].filter(line => line !== '').join('\n');

    return { generatedAt, dueSoonCount: dueSoon.length, overdueCount: overdue.length, priorityFilters, deadlineClusters, highLeverage, markdown };
  }

  function buildDeadlineStudyPack(items, now = new Date()) {
    const sourceItems = (Array.isArray(items) ? items : [items]).filter(Boolean);
    const generatedAt = normalizeBriefDate(now).toISOString();
    const cards = sourceItems.slice(0, 20).map((item, index) => {
      ...
      const freshnessWarnings = inferFreshnessWarnings(item, Date.parse(generatedAt));
      const reviewSchedule = buildStudyPackReviewSchedule(item, dueAt, generatedAt);
      const workflowLabels = inferCsWorkflowTypeLabels(item);
      const tags = [
    ...
    const markdown = [
      ...
      ...cards.flatMap(card => [
        ...
        card.freshnessWarnings.length ? `Freshness: ${card.freshnessWarnings.join('; ')}` : '',
        card.citation ? `Citation: ${card.citation}` : '',
        card.workflowLabels.length ? `Workflow: ${card.workflowLabels.join(', ')}` : '',
        ...
        card.reviewSchedule.length ? 'Review schedule:' : '',
        ...card.reviewSchedule.map(step => `- ${step}`),
        ''
      ].filter(line => line !== ''))
    ].join('\n');
  ...
  function inferSubmissionChecklist(item) {
    ...
    if (/\b(ai tutor|ai study|study mode|guided learning|notebooklm|chatgpt|claude|gemini|perplexity|quizlet|knowt|studyfetch|turbolearn|mindgrasp|gizmo|raena|thea|studdy|jotbot|studyblaze|quizard|anki|flashcards?|spaced repetition)\b/.test(source)) {
      add('verify AI study output against source');
    }
    if (/\b(ai[- ]?generated code|prompt[- ]?to[- ]?code|vibe cod(?:e|ing)|copilot|cursor|windsurf|replit agent|bolt\.new|v0\.dev|lovable\.dev)\b/.test(source)) {
      add('review generated code before submit');
    }
  ...
  function inferNextActionLabel(item, nowMs = Date.now()) {
    const bucket = classifyActionBucket(item, nowMs);
    const { workflowState, hasSubmission, gradedAt, score } = getSubmissionSnapshot(item);
    const source = `${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const ts = Number(item && item.ts);
    const hoursUntilDue = Number.isFinite(ts) ? (ts - nowMs) / (60 * 60 * 1000) : Infinity;

    if (bucket === 'submitted') {
      return gradedAt || score != null || /graded|complete/.test(workflowState) ? 'review feedback' : 'waiting on grade';
    }
    if (bucket === 'overdue') return hasSubmission ? 'verify late status' : 'submit or message instructor';
    if (/\b(feedback|grade comments?|rubric|regrade|corrections?|revise and resubmit)\b/.test(source)) return 'review feedback';
    if (/\b(submit|upload|attach|gradescope|autograder|canvas|turn in|turn-in|due)\b/.test(source) && hoursUntilDue <= 24) return 'needs submission';
    if (/\b(project|lab|paper|essay|presentation|exam|midterm|final|capstone|portfolio|milestone|implementation)\b/.test(source) && hoursUntilDue <= 72) return 'start now';
    if (bucket === 'due soon') return 'plan study block';
    if (bucket === 'no due date') return 'clarify due date';
    return 'low priority';
  }

  function isCsWorkflowItem(item) {
    const source = `${item?.courseName || item?.course || ''} ${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    return /\b(cs|cse|cis|computer science|programming|coding|code|lab|project|repo(?:sitory)?|github|git\b|pull request|pr\b|autograder|gradescope|submitty|codegrade|pytest|unit tests?|debug(?:ging)?|algorithm|data structure|runtime|big[- ]?o|compiler|database|sql|operating systems?|machine learning|neural network)\b/.test(source);
  }

  function inferCsWorkflowTypeLabels(item) {
    const source = `${item?.courseName || item?.course || ''} ${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const labels = [];
    const add = (label) => { if (label && !labels.includes(label)) labels.push(label); };

    if (/\b(implement(?:ation)?|coding|programming|starter code|repo(?:sitory)?|github|git\b|pull request|pr\b|commit|branch|api|frontend|backend|full[- ]?stack)\b/.test(source)) add('coding');
    if (/\b(debug(?:ging)?|bug(?:s)?|failing tests?|test cases?|unit tests?|pytest|jest|xcodebuild|junit|coverage|trace|stack trace|minimal repro)\b/.test(source)) add('debug/test');
    if (/\b(gradescope|autograder|auto[- ]?grader|submitty|codegrade|hidden tests?|public tests?)\b/.test(source)) add('autograder');
    if (/\b(read(?:ing)?|chapter|lecture|slides?|notes?|transcript|video|paper|article)\b/.test(source)) add('reading/review');
    if (/\b(quiz|exam|midterm|final|practice exam|study guide|flashcards?|anki|spaced repetition)\b/.test(source)) add('exam prep');
    if (/\b(write[- ]?up|report|readme|reflection|design doc|implementation notes?|memo|essay|paper)\b/.test(source)) add('write-up');
    if (/\b(project|milestone|checkpoint|capstone|portfolio|presentation|demo)\b/.test(source)) add('project');

    return labels.slice(0, 4);
  }

  function summarizePriorityFilters(items, nowMs = Date.now()) {
    const sourceItems = (Array.isArray(items) ? items : [items]).filter(Boolean);
    const counts = {
      overdue: 0,
      dueSoon: 0,
    ...
    for (const item of sourceItems) {
      ...
      else if (bucket === 'due soon') counts.dueSoon++;
      else if (bucket === 'submitted') counts.submitted++;
      else if (bucket === 'no due date') counts.noDueDate++;
      else counts.later++;

      if (isCsWorkflowItem(item)) counts.csWorkflow++;
      if (bucket === 'overdue' || (bucket === 'due soon' && (classifyDeadline(item, nowMs).effort === 'high' || inferPlannerRiskFlags(item, sourceItems, nowMs).length))) {
  ...
  function formatPriorityFilterSummary(counts) {
    return [
      `Needs attention ${counts.needsAttention}`,
      `Overdue ${counts.overdue}`,
      `Due soon ${counts.dueSoon}`,
      `CS/lab ${counts.csWorkflow}`,
      `Submitted ${counts.submitted}`
    ].join(' · ');
  }

  function summarizeDeadlineClusters(items, nowMs = Date.now()) {
    const sourceItems = (Array.isArray(items) ? items : [items]).filter(Boolean);
    const byDay = new Map();
    for (const item of sourceItems) {
      const ts = Number(item && item.ts);
      if (!Number.isFinite(ts) || item.done) continue;
      const dayKey = new Date(ts).toDateString();
      const bucket = byDay.get(dayKey) || { dayKey, ts, count: 0, highRisk: 0, csWorkflow: 0, titles: [] };
      bucket.ts = Math.min(bucket.ts, ts);
      bucket.count += 1;
      if (isCsWorkflowItem(item)) bucket.csWorkflow += 1;
      if (classifyDeadline(item, nowMs).effort === 'high' || inferPlannerRiskFlags(item, sourceItems, nowMs).length) bucket.highRisk += 1;
      const title = String(item.title || 'Canvas item').trim();
      if (title && bucket.titles.length < 3) bucket.titles.push(title);
      byDay.set(dayKey, bucket);
    }

    return Array.from(byDay.values())
      .filter(cluster => cluster.count >= 2 || cluster.highRisk >= 1)
      .sort((a, b) => b.highRisk - a.highRisk || b.count - a.count || a.ts - b.ts)
      .slice(0, 3)
      .map(cluster => ({
        date: new Date(cluster.ts).toISOString().slice(0, 10),
        count: cluster.count,
        highRisk: cluster.highRisk,
        csWorkflow: cluster.csWorkflow,
        titles: cluster.titles,
        summary: `${new Date(cluster.ts).toISOString().slice(0, 10)}: ${cluster.count} due${cluster.highRisk ? `, ${cluster.highRisk} high-risk` : ''}${cluster.csWorkflow ? `, ${cluster.csWorkflow} CS/lab` : ''}`
      }));
  }
  ...
  function inferRubricFeedbackDigest(item) {
    const source = `${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const sections = [];
    const add = (label) => { if (!sections.includes(label)) sections.push(label); };

    if (/\b(full credit|excellent|great job|well done|strong|correct|passed|meets expectations|complete)\b/.test(source)) {
      add('What went well: preserve the approach that earned credit');
    }
    if (/\b(partial credit|deduct(?:ed|ion)?|lost points?|missing|incorrect|wrong|failed|needs work|incomplete|revise|revision|resubmit)\b/.test(source)) {
      add('Fix next time: convert deductions into a short correction checklist');
    }
    if (/\b(rubric|criteria|learning objectives?|objective|outcome|concept|topic|chapter|lecture|slides?|notes?)\b/.test(source)) {
      add('Review next: tie each comment back to the matching rubric criterion or course topic');
    }
    if (/\b(test cases?|hidden tests?|autograder|gradescope|assert(?:ion)?|edge cases?|boundary|timeout|memory limit)\b/.test(source)) {
      add('Evidence to keep: save failing tests, edge cases, and the final passing run');
    }
    if (/\b(feedback|comments?|score|points?|grade|graded)\b/.test(source) && sections.length === 0) {
      add('Feedback loop: summarize what went well, what to fix, and what to review before the next attempt');
    }
  ...
  function inferPostLectureCleanupHints(item) {
    const source = `${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const hints = [];
    const add = (label) => { if (!hints.includes(label)) hints.push(label); };

    if (/\b(lecture|class notes?|slides?|recording|transcript|caption(?:s)?|seminar|recitation)\b/.test(source)) {
      add('clean messy notes into outline');
    }
    if (/\b(action items?|todos?|to-dos?|follow[- ]?ups?|homework|assignment|due|deadline|next steps?)\b/.test(source)) {
      add('extract follow-up tasks');
    }
    if (/\b(confus(?:ed|ing|ion)|unclear|muddiest point|questions?|stuck|weak spots?|don'?t understand)\b/.test(source)) {
      add('separate confusion list');
    }
    if (/\b(ai note[- ]?tak(?:er|ing)|notebooklm|chatgpt|claude|gemini|llm|assistant|summary|summari[sz]e|cleanup|clean up)\b/.test(source)) {
      add('verify AI cleanup against source');
    }
  ...
  function inferAgenticCodingCheckpointHints(item) {
    const source = `${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const hints = [];
    const add = (label) => { if (!hints.includes(label)) hints.push(label); };

    if (/\b(agentic coding|coding agent|ai agent|autonomous coding|claude code|codex|cursor agent|cursor composer|windsurf cascade|copilot workspace|github copilot|replit agent|bolt\.new|lovable\.dev|v0\.dev|prompt[- ]?to[- ]?code|vibe cod(?:e|ing))\b/.test(source)) {
      add('checkpoint before agent edits');
      add('constrain files and tests');
    }
    if (/\b(diff|patch|pull request|pr\b|commit|branch|merge|review generated code|model changes?|generated patch)\b/.test(source)) {
      add('inspect agent diff');
    }
    if (/\b(test cases?|unit tests?|pytest|junit|npm test|autograder|gradescope|ci\b|build)\b/.test(source)) {
      add('run verification loop');
    }
  ...
  function inferExamSignalHints(item) {
    const source = `${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const hints = [];
    const add = (label) => { if (!hints.includes(label)) hints.push(label); };

    if (/\b(on|for|in)\s+the\s+(exam|midterm|final|quiz|test)\b|\bexam[- ]?relevant\b|\btestable\b/.test(source)) {
      add('pin likely testable clue');
    }
    if (/\b(important|key idea|key concept|must know|you should know|need to know|don'?t forget|remember this)\b/.test(source)) {
      add('prioritize professor emphasis');
    }
    if (/\b(common mistake|common error|pitfall|trick question|watch out|easy to miss|students often)\b/.test(source)) {
      add('practice common mistake');
    }
    if (/\b(learning objectives?|study guide|review sheet|exam review|practice problems?|sample questions?)\b/.test(source)) {
      add('convert guide into recall questions');
    }
  ...
  function inferNextClassPrepHints(item, nowMs = Date.now()) {
    ...
    if (hoursUntilDue >= 0 && hoursUntilDue <= 36 && /\b(class|lecture|section|lab|seminar|workshop|recitation|studio)\b/.test(source)) {
      add('prep before next class');
    }
    if (/\b(read(?:ing)?|chapter|pages?|skim|slides?|lecture notes?|module|video|recording)\b/.test(source)) {
      add('skim assigned material');
    }
    if (/\b(question|discussion|participation|bring|prepare|office hours|recitation|seminar)\b/.test(source)) {
      add('write one question to bring');
    }
    if (/\b(download|print|worksheet|handout|template|starter|notebook|dataset|materials?)\b/.test(source)) {
      add('stage class materials');
    }
  ...
  function inferLiveStudyModeHints(item) {
    const source = `${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const hints = [];
    const add = (label) => { if (!hints.includes(label)) hints.push(label); };
    const isLiveTutor = /\b(live study|live mode|voice mode|spoken tutor|audio tutor|ai tutor|study mode|guided learning|learning coach|chatgpt|gemini|claude|khanmigo|notebooklm|coconote|quizlet|knowt|studyfetch|flashcards?|self[- ]?quiz)\b/.test(source);

    if (isLiveTutor && /\b(voice|audio|spoken|explain aloud|talk through|oral|conversation|dialogue|live)\b/.test(source)) {
      add('practice aloud with live tutor');
    }
    if (isLiveTutor && /\b(hint(?:s)?|guided|socratic|step[- ]?by[- ]?step|coach|attempt first|show (?:my )?work|scratch work|no direct answers?)\b/.test(source)) {
      add('ask for hints before answers');
    }
    if (isLiveTutor && /\b(confidence|rate|calibrat(?:e|ion)|red\/yellow\/green|traffic light|weak spots?|miss(?:ed|es)|mistakes?|wrong answers?)\b/.test(source)) {
      add('rate confidence after each response');
    }
    if (isLiveTutor && /\b(transcript|record(?:ing)?|summary|notes?|citations?|source(?:s)?|slides?|textbook|lecture)\b/.test(source)) {
      add('save tutor transcript with sources');
    }

    return hints.slice(0, 3);
  }

  function inferStudyModeRoutingHints(item) {
    const source = `${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const hints = [];
    const add = (label) => { if (!hints.includes(label)) hints.push(label); };
    const mentionsAiStudyTool = /\b(ai study|study mode|ai tutor|learning coach|notebooklm|coconote|quizlet|knowt|studyfetch|study guide|flashcards?|audio overview|audio recap|podcast|mind map|concept map|practice questions?|self[- ]?quiz)\b/.test(source);

    if (!mentionsAiStudyTool) return hints;
    if (/\b(source-grounded|grounded|citations?|evidence|assignment spec|rubric|lecture notes?|slides?|transcript|textbook|reading|paper|article|pdf|uploaded sources?)\b/.test(source)) {
      add('route to source-grounded Q&A');
    }
    if (/\b(flashcards?|anki|quizlet|spaced repetition|terms?|definitions?|vocab(?:ulary)?)\b/.test(source)) {
      add('route to flashcard drill');
    }
    if (/\b(practice questions?|practice problems?|problem set|pset|self[- ]?quiz|mock exam|sample exam|quiz|exam|midterm|final)\b/.test(source)) {
      add('route to practice quiz');
    }
    if (/\b(mind map|concept map|knowledge graph|timeline|compare(?: and contrast)?|relationships?|connections?)\b/.test(source)) {
      add('route to concept map');
    }
    if (/\b(audio overview|audio recap|podcast|commute review|listen(?:ing)?|voice summary|spoken recap)\b/.test(source)) {
      add('route to audio recap');
    }

    return hints.slice(0, 3);
  }

  function inferSourceLinkedReviewCardHints(item) {
    const source = `${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const hints = [];
    const add = (label) => { if (!hints.includes(label)) hints.push(label); };
    const asksForReviewCards = /\b(review cards?|flashcards?|anki|quizlet|spaced repetition|recall cards?|self[- ]?test|practice questions?)\b/.test(source);

    if (!asksForReviewCards) return hints;
    if (/\b(page(?:s)?|slide(?:s)?|timestamp(?:s)?|citation(?:s)?|source(?:s)?|transcript|pdf|reading|lecture notes?|highlights?)\b/.test(source)) {
      add('keep source anchor on each card');
    }
    if (/\b(question|prompt|front side|answer|back side|cloze|definition|term|concept)\b/.test(source)) {
      add('split prompt and answer cleanly');
    }
    if (/\b(confus(?:ed|ing|ion)|weak spots?|missed|wrong answers?|mistake(?:s)?|hard topics?|low confidence)\b/.test(source)) {
      add('tag weak-concept cards');
    }
    if (/\b(export|copy|markdown|csv|anki|quizlet|share|download)\b/.test(source)) {
      add('export portable card deck');
    }
  ...
  function inferLectureEmphasisHints(item) {
    const source = `${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const hints = [];
    const add = (label) => { if (!hints.includes(label)) hints.push(label); };
    const isLectureArtifact = /\b(lecture|class recording|recorded class|transcript|transcription|caption(?:s)?|slides?|seminar|review session|panopto|kaltura|echo360|notebooklm|coconote|ai note[- ]?tak(?:er|ing)|auto[- ]?notes?)\b/.test(source);

    if (isLectureArtifact && /\b(emphas(?:is|ize|ized)|important|key point|highlight(?:ed)?|called out|stressed|focus on|pay attention|remember this|will be on (?:the )?(?:exam|quiz|midterm|final))\b/.test(source)) {
      add('capture instructor emphasis');
    }
    if (isLectureArtifact && /\b(repeated(?:ly)?|again|recurring|came back to|several times|multiple times|theme(?:s)?|pattern(?:s)?)\b/.test(source)) {
      add('promote repeated themes');
    }
    if (isLectureArtifact && /\b(not tested|skip|less important|optional|extra credit|don'?t worry|out of scope|won'?t be on (?:the )?(?:exam|quiz|midterm|final))\b/.test(source)) {
      add('separate low-priority material');
    }
    if (isLectureArtifact && /\b(example(?:s)?|demo|walkthrough|worked problem|case study|sample question|practice problem)\b/.test(source)) {
      add('turn instructor examples into practice');
    }
  ...
  function inferRubricRevisionLoopHints(item) {
    const source = `${item?.title || ''} ${item?.description || item?.text || item?.content || ''}`.toLowerCase();
    const hints = [];
    const add = (label) => { if (!hints.includes(label)) hints.push(label); };

    if (/\b(revise|revision|resubmit|resubmission|redo|correction(?:s)?|regrade|grade appeal|retake|make[- ]?up|second attempt|retry)\b/.test(source)) {
      add('compare feedback to rubric');
    }
    if (/\b(feedback|comments?|markup|annotation(?:s)?|rubric|criteria|score breakdown|lost points?|deduct(?:ed|ion)|partial credit)\b/.test(source)) {
      add('list lost-point causes');
    }
    if (/\b(before and after|change log|revision memo|reflection|cover note|explain changes|track changes|diff|commit history)\b/.test(source)) {
      add('write revision memo');
    }
    if (/\b(office hours|ta\b|professor|instructor|grader|peer review|review session)\b/.test(source)) {
      add('confirm regrade question');
    }
  ...
  function buildPlannerPrompt(deadlines, now = new Date()) {
    ...
    const lines = deadlines.slice(0, 15).map(d => {
      ...
      const postLectureCleanupHints = inferPostLectureCleanupHints(d);
      const aiHandoffHints = inferAiHandoffHints(d);
      const csWorkflowHints = inferCsWorkflowHints(d);
      const agenticCodingHints = inferAgenticCodingCheckpointHints(d);
      ...
      const examSignalHints = inferExamSignalHints(d);
      ...
      const nextClassPrepHints = inferNextClassPrepHints(d, nowMs);
      ...
      const liveStudyModeHints = inferLiveStudyModeHints(d);
      const studyModeRoutingHints = inferStudyModeRoutingHints(d);
      const sourceLinkedReviewCardHints = inferSourceLinkedReviewCardHints(d);
      const lectureChapteringHints = inferLectureChapteringHints(d);
      const lectureEmphasisHints = inferLectureEmphasisHints(d);
      const discussionReplyPrepHints = inferDiscussionReplyPrepHints(d);
      const rubricRevisionHints = inferRubricRevisionLoopHints(d);
      ...
      const postLectureCleanupHint = postLectureCleanupHints.length ? `; post-lecture cleanup: ${postLectureCleanupHints.join(', ')}` : '';
      ...
      const agenticCodingHint = agenticCodingHints.length ? `; agentic coding checkpoint: ${agenticCodingHints.join(', ')}` : '';
      ...
      const examSignalHint = examSignalHints.length ? `; exam signals: ${examSignalHints.join(', ')}` : '';
      ...
      const nextClassPrepHint = nextClassPrepHints.length ? `; next-class prep: ${nextClassPrepHints.join(', ')}` : '';
      ...
      const liveStudyModeHint = liveStudyModeHints.length ? `; live study mode: ${liveStudyModeHints.join(', ')}` : '';
      const studyModeRoutingHint = studyModeRoutingHints.length ? `; study mode route: ${studyModeRoutingHints.join(', ')}` : '';
      const sourceLinkedReviewCardHint = sourceLinkedReviewCardHints.length ? `; source-linked review cards: ${sourceLinkedReviewCardHints.join(', ')}` : '';
      const lectureChapteringHint = lectureChapteringHints.length ? `; lecture chaptering: ${lectureChapteringHints.join(', ')}` : '';
      const lectureEmphasisHint = lectureEmphasisHints.length ? `; lecture emphasis: ${lectureEmphasisHints.join(', ')}` : '';
      const discussionReplyPrepHint = discussionReplyPrepHints.length ? `; discussion reply prep: ${discussionReplyPrepHints.join(', ')}` : '';
      const rubricRevisionHint = rubricRevisionHints.length ? `; revision loop: ${rubricRevisionHints.join(', ')}` : '';
      ...
      const hint = `urgency=${triage.urgency}, effort=${triage.effort}${actionBucketHint}${resourceLinkHint}${checklistHint}${reviewHint}${weakConceptHint}${learningHint}${focusHint}${energyHint}${practiceHint}${retrievalHint}${socraticHint}${teachBackHint}${integrityHint}${privacyConsentHint}${accessibilityStudyHint}${codeDebugHint}${minimalReproHint}${autograderFeedbackHint}${officeHoursHint}${collaborationHint}${lectureHint}${lectureActionHint}${sourceCoverageHint}${aiNoteQualityHint}${confusionCaptureHint}${audioReviewHint}${transcriptStudyGuideHint}${multimodalHint}${sourceGroundingHint}${evidencePackHint}${tutorContextHint}${feedbackLoopHint}${portabilityHint}${wrapUpHint}${rubricHint}${preSubmitHint}${submissionReceiptHint}${availabilityWindowHint}${notebookStudyPackHint}${studyPackArtifactHint}${threeTwoOneReviewHint}${readingTriageHint}${lectureQuestionHint}${postLectureCleanupHint}${aiHandoffHint}${csWorkflowHint}${agenticCodingHint}${commandSnippetHint}${assignmentSpecHint}${milestoneDecompositionHint}${requirementClarificationHint}${activePracticeHint}${interleavedPracticeHint}${metacognitiveHint}${spacedReviewHint}${examCountdownHint}${examConstraintHint}${examSignalHint}${peerAccountabilityHint}${recurringRoutineHint}${timeEstimateHint}${blockedDependencyHint}${workedExampleHint}${evidenceConfidenceHint}${changeAwarenessHint}${specDeltaHint}${questionBankHint}${aiQuizGenerationHint}${freshnessGuardHint}${dueDateAmbiguityHint}${nextClassPrepHint}${dueDateConfidenceHint}${hiddenDeadlineHint}${aiStudySessionHint}${personalizedMemoryHint}${errorNotebookHint}${notebookLmStudyPlanHint}${conceptMapBridgeHint}${firstStudyStepHint}${studyRecoveryHint}${calendarConflictHint}${assignmentQuestionHint}${questionFirstNoteHint}${missedLectureCatchUpHint}${aiSourceBoundaryHint}${liveStudyModeHint}${studyModeRoutingHint}${sourceLinkedReviewCardHint}${lectureChapteringHint}${lectureEmphasisHint}${discussionReplyPrepHint}${rubricRevisionHint}${ambiguousTitleHint}${gradeImpactHint}${phaseHint}${executionPlanHint}${riskHint}`;
  ...
  function renderDeadlineList(items) {
    ...
    items.slice(0, 12).forEach((item, i) => {
      ...
      const agenticCodingHints = inferAgenticCodingCheckpointHints(item);
      ...
      const rubricRevisionHints = inferRubricRevisionLoopHints(item);
      ...
      const reviewLabel = [
        ...
        agenticCodingHints.length ? `Agent checkpoint: ${agenticCodingHints.join(' · ')}` : '',
        ...
        rubricRevisionHints.length ? `Revision loop: ${rubricRevisionHints.join(' · ')}` : '',
